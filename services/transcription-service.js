...
class TranscriptionService extends EventEmitter {
  constructor() {
    ...
    this.dgConnection = deepgram.listen.live({
      encoding: 'mulaw',
      sample_rate: '8000',
      model: 'whisper',  // Using Deepgram's Whisper model
      language: 'ar',    // Primary language set to Arabic
      detect_language: true, // Enable language detection for code-switching
      punctuate: true,
      interim_results: true,
      endpointing: 200,
      utterance_end_ms: 1000,
      multichannel: false,
      alternatives: 1,
      smart_format: true // Helps with formatting numbers and punctuation
    });

    this.finalResult = '';
    this.speechFinal = false;
    this.currentLanguage = 'ar'; // Track current language
    ...
    this.dgConnection.on(LiveTranscriptionEvents.Open, () => {
      this.dgConnection.on(LiveTranscriptionEvents.Transcript, (transcriptionEvent) => {
        ...
        if (alternatives) {
          text = alternatives[0]?.transcript || '';
          detectedLanguage = alternatives[0]?.language || 'ar';
        }

        // Log language detection for monitoring
        if (detectedLanguage !== this.currentLanguage) {
          console.log(`Language switch detected: ${this.currentLanguage} -> ${detectedLanguage}`.cyan);
          this.currentLanguage = detectedLanguage;
        }
        
        if (transcriptionEvent.type === 'UtteranceEnd') {
          if (!this.speechFinal) {
            console.log(`UtteranceEnd [${this.currentLanguage}]: ${this.finalResult}`.yellow);
            this.emit('transcription', this.finalResult, this.currentLanguage);
            return;
          } else {
            console.log(`STT -> Speech was already final when UtteranceEnd received [${this.currentLanguage}]`.yellow);
            return;
          }
        }
        ...
        if (transcriptionEvent.is_final === true && text.trim().length > 0) {
          ...
          if (transcriptionEvent.speech_final === true) {
            ...
            console.log(`Final transcription [${this.currentLanguage}]: ${this.finalResult}`.green);
            this.emit('transcription', this.finalResult, this.currentLanguage);
            this.finalResult = '';
          } else {
            this.speechFinal = false;
          ...
          this.emit('utterance', text, this.currentLanguage);
        }
      });
      ...
      this.dgConnection.on(LiveTranscriptionEvents.Metadata, (metadata) => {
        if (metadata.language_id) {
          console.log(`Language identification: ${metadata.language_id}`.cyan);
        }
      });
  ...
  send(payload) {
    if (this.dgConnection.getReadyState() === 1) {
      try {
        ...
        console.error('Error sending audio to Deepgram:'.red);
        console.error(error);
        this.emit('error', error);
      }
    }
